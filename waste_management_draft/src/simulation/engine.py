...
# Orchestrates the waste collection lifecycle, calling routing, prioritisation, vehicle, and facility allocations.

import uuid
from simulation.monitoring import WasteMonitor
from simulation.priority import PriorityScoring
from simulation.vehicle_allocation import VehicleAllocator
from facilities.facility_allocation import FacilityAllocator
from simulation.input_processor import calculate_penalty
...
class SimulationEngine:
    ...
    def add_waste(self, bin_id, waste_type, quantity, user_id=None):
        """
        Adds waste to a specific bin and logs the event in CLI and persistent database.
        """
        ...
        if result and result.get("success"):
            ...
            logger.log_event({
                ...
                "user_id": user_id or "SimUser",
                "category": waste_type,
                "quantity": quantity,
                "contamination": contamination_qty,
                "penalty": penalty
            })
    ...
    def run_optimization(self, alert_threshold=80.0):
        ...
        for bin_obj, score in ranked:
            ...
            # Log completion to central route 完成 log
            logger.log_route_event(
                vehicle.vehicle_id, 
                " -> ".join(route_nodes), 
                route_distance, 
                len(bins_to_collect), 
